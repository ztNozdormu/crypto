...
impl Chacha20Poly1305 {
    ...
    pub fn encrypt_slice(&mut self, pkt_seq_num: u32, aead_pkt: &mut [u8]) {
        ...
        let (pkt_len, plaintext_and_ciphertext_and_tag) = aead_pkt.split_at_mut(Self::PKT_OCTETS_LEN);

        let plen = plaintext_and_ciphertext_and_tag.len() - Self::TAG_LEN;
        let (plaintext_and_ciphertext, tag_out) = plaintext_and_ciphertext_and_tag.split_at_mut(plen);

        self.encrypt_slice_detached(pkt_seq_num, pkt_len, plaintext_and_ciphertext, tag_out)
    }

    pub fn decrypt_slice(&mut self, pkt_seq_num: u32, aead_pkt: &mut [u8]) -> bool {
        debug_assert!(aead_pkt.len() >= Self::TAG_LEN + Self::PKT_OCTETS_LEN);

        let (pkt_len, ciphertext_and_plaintext_and_tag) = aead_pkt.split_at_mut(Self::PKT_OCTETS_LEN);

        let clen = ciphertext_and_plaintext_and_tag.len() - Self::TAG_LEN;
        let (ciphertext_and_plaintext, tag_in) = ciphertext_and_plaintext_and_tag.split_at_mut(clen);

        self.decrypt_slice_detached(pkt_seq_num, pkt_len, ciphertext_and_plaintext, &tag_in)
    }

    pub fn encrypt_slice_detached(&mut self, pkt_seq_num: u32, pkt_len: &mut [u8], plaintext_and_ciphertext: &mut [u8], tag_out: &mut [u8]) {
        // let (pkt_len, plaintext_and_ciphertext) = plaintext_and_ciphertext.split_at_mut(Self::PKT_OCTETS_LEN);
        ...
        self.c2.encrypt_slice(pkt_seq_num, 1, plaintext_and_ciphertext);
        ...
        // padding
        let r = Poly1305::BLOCK_LEN - (pkt_len.len() + plen) % Poly1305::BLOCK_LEN;
        if r > 0 {
    ...
    pub fn decrypt_slice_detached(&mut self, pkt_seq_num: u32, pkt_len: &mut [u8], ciphertext_and_plaintext: &mut [u8], tag_in: &[u8]) -> bool {
        // let alen = aad.len();
        ...
        // padding
        let r = Poly1305::BLOCK_LEN - (pkt_len.len() + clen) % Poly1305::BLOCK_LEN;
        if r > 0 {
            poly1305.update(&Self::PADDING_BLOCK[..r]);
        }
        let tag = poly1305.finalize();
        ...
        if is_match {
            ...
            // Set Chacha's block counter to 1
            self.c2.decrypt_slice(pkt_seq_num, 1, ciphertext_and_plaintext);
        }
